...
/*Consultar huespedes*/
router.get('/view/:guest_id', async (req, res) => {
    const { guest_id } = req.params;
    const guest = await pool.query('select * from huesped where guest_id=?', [guest_id]);
...
//Eliminar huesped
router.get('/delete/:guest_id', async (req, res) => {
    const { guest_id } = req.params;
