...
router.get('/', async (req, res) => {
    if (req.user.user === 0) {
        ...
        const userlist = await pool.query("SELECT `worker`.`worker_id`, `worker`.`worker_code`, `worker`.`name`, `worker`.`lastname`, `worker`.`username`, `worker`.`datereg`, `role`.`role` FROM `worker` LEFT JOIN `role` ON `worker`.`role` = `role`.`role_id` WHERE `worker`.`status` != '0' AND `worker`.`account` = '1'");
